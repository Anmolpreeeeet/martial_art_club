...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public void deleteMartialArtObjectFromDatabaseById(int id){
        SQLiteDatabase database = getWritableDatabase();
        String deleteMartialArtsSQLCommand = "delete from " + MARTIAL_ARTS_TABLE +
                                             " where " + ID_KEY + " = " + id;
        database.execSQL(deleteMartialArtsSQLCommand);
        database.close();
    }
    ...
    public MartialArt returnMartialArtObjectById(int id){
        ...
        if(cursor.moveToFirst()){
            martialArtObject = new MartialArt(cursor.getInt(0),
                    cursor.getString(1), cursor.getDouble(2), cursor.getString(3));
        }
        database.close();
